...
public class LC113PathSumIi {
    ...
    class Solution {
        ...
        public void backtrack(TreeNode root, int target, List<List<Integer>> result, List<Integer> list) {
            if (root == null) return;
            list.add(root.val);

            if (root.left == null && root.right == null) {
                if (root.val == target) result.add(new ArrayList<>(list));
                list.remove(list.size() - 1);
                return;
            }

            backtrack(root.left, target - root.val, result, list);
            backtrack(root.right, target - root.val, result, list);
            list.remove(list.size() - 1);
        }
    }
