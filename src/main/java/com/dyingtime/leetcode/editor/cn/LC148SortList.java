...
public class LC148SortList {
    ...
    public static void main(String[] args) {
        Solution solution = new LC148SortList.Solution();
    }
    ...
    class Solution {
        ...
        //         head = head.next;
        //     }
        //     Integer[] objects = values.toArray(new Integer[0]);
        //     Arrays.sort(objects);
        //     ListNode root = new ListNode(objects[0]);
        //     ListNode current = root;
        //     for (int i = 1; i < objects.length; i++) {
        //         current.next = new ListNode(objects[i]);
        ...
        public ListNode sortList(ListNode head) {
            ...
            for (int subLen = 1; subLen < length; subLen <<= 1) { // subLen每次左移一位（即sublen = sublen*2） PS:位运算对CPU来说效率更高
                ...
                while (curr != null) {               // 如果链表没有被拆完
                    ...
                    prev.next = mergeTwoLists(head_1, head_2);        // prev.next 指向排好序链表的头
